...
class BarCodeReaderTest(unittest.TestCase):
    ...
    def test_list_is_not_empty(self):
        (bar_codes, bar_codes_without_order) = BarCodeReader.read(TEST_ASSETS_DIRECTORY + "one_barcode.csv")
    ...
    def test_read_multiple_barcodes(self):
        (bar_codes, bar_codes_without_order) = BarCodeReader.read(TEST_ASSETS_DIRECTORY + "barcodes.csv")
        self.assertEquals(len(bar_codes), 7)
        self.assertEquals(len(bar_codes_without_order), 0)

    def test_orders_without_barcodes_are_not_included(self):
        (bar_codes, bar_codes_without_order) = BarCodeReader.read(TEST_ASSETS_DIRECTORY + "some_incorrect_barcodes.csv")
    ...
    def test_any_bar_code_repeated_has_been_included(self):
        (bar_codes, bar_codes_without_order) = BarCodeReader.read(TEST_ASSETS_DIRECTORY + "barcodes_with_duplicates.csv")
        self.assertEquals(len(bar_codes), 5)
        self.assertEquals(len(bar_codes_without_order), 0)
