...
class ConsoleOutputTest(unittest.TestCase):
    ...
    def test_exception_when_type_is_incorrect(self):
        self.assertRaises(InvalidObjectTypeException, ConsoleOutput.render_customers_with_bar_codes, 1)

    def test_exception_when_items_belongs_to_incorrect_type(self):
        self.assertRaises(InvalidObjectTypeException, ConsoleOutput.render_customers_with_bar_codes, {"1": "Type"})
    ...
    def test_when_list_has_a_customer_without_barcodes(self):
        self.__assert_console_result(
            [
                Customer("1", [OrderWithBarCodes("2", "1", [])])
            ],
    ...
    def test_when_list_has_a_customer_with_barcodes(self):
        self.__assert_console_result(
            [
                Customer("1", [OrderWithBarCodes("2", "1", [1111,2222,33333])])
            ],
    ...
    def test_when_list_has_a_customer_with_multiple_barcodes(self):
        self.__assert_console_result(
            [
                Customer("1", [OrderWithBarCodes("2", "1", [1111, 2222, 33333])]),
                Customer("2", [OrderWithBarCodes("5", "2", [4444, 5555, 6666])]),
                Customer("3", [OrderWithBarCodes("6", "3", [7777, 8888, 9999])]),
            ],
    ...
    def __assert_console_result(self, input, output_file):
        ...
        ConsoleOutput.render_customers_with_bar_codes(input)
